...
It is not going to use any server, such as [Next.js](https://nextjs.org/), which do give you more option, but add extra steps at deployment.
...
To fix this error, use the solution described in [Adding TypeScript](https://mdxjs.com/advanced/typescript).

3. Create `mdx.d.ts` in the `src` directory, and add the following content.  
...
Note that the `code` component had no effect when used directly, without `MDXProvider`.
Using `MDXProvider`, it worked as expected.
